...
QDRANT_URL = (
    os.getenv("QDRANT_URL") or os.getenv("qdrant_url") or "http://localhost:6333"
)
POSTGRES_URL = (
...
def run_interventions_pipeline(
    csv_path: str = "data/interventions.csv", force_db_load: bool = False
) -> None:
...
def run_procedures_pipeline(
    input_dir: str = "data/procedures",
    output_dir: str = "data/procedures_extracted",
    chunks_csv: str = "data/procedure_chunks.csv",
) -> None:
    ...
    parse_procedures(input_dir=Path(input_dir), output_dir=Path(output_dir))

    # 2. Extract semantic chunks and enrich with LLM context
    print("\n[Step 2/3] Extracting and enriching chunks to CSV...")
    extract_procedures(json_dir=Path(output_dir), output_csv=Path(chunks_csv))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Hephaestus Data Engineering Pipeline Orchestrator"
    )
    parser.add_argument(
        "pipeline",
