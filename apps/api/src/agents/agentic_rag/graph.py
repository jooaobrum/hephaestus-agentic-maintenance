...
from agents.agentic_rag.agents import (
    agent_node,
    intent_router_node,
    UsedReference,
    ALL_TOOLS,
    ...
    if is_node_start:
        ...
        if "intent_router_node" in update:
            ...
            if data.get("question_relevant"):
                event = {
                    "event": "status",
                    "data": "Question is relevant, proceeding to agent",
                }
            else:
                event = {
                    "event": "status",
                    "data": f"Question not relevant: {data.get('answer', '')}",
                }
                ...
                if hasattr(msg, "tool_calls") and msg.tool_calls:
                    ...
                    if data.get("final_answer"):
                        return {
                            ...
                            "data": msg.content,
                            "references": [
                                ref.model_dump() for ref in data.get("references", [])
                            ],
                        }
